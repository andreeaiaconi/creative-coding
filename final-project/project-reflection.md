...
The contour segments were grouped by noise visibility into three buckets for performance, and drawn as stroked lines with a neon glow (using `shadowBlur`). The noise sweep from attempt 1 carried over — `random.noise3D` controlled whether each segment was visible at any given frame.
